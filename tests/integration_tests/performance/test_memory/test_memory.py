...
COMPILE_CMD = 'gcc -fopenmp -mcmodel=medium -O3 -march=znver1 -DSTREAM_ARRAY_SIZE={} -DNTIMES={} -DOFFSET={} {} -o {}'
...
def prepare_binary(local_test_path, vcpus=1):
...
def clear_cache(local_test_path, numa_node):
    """Clear the cache on the host."""
    clear_cache_path = os.path.join(local_test_path, 'clear_cache');
    proc = run('./{path} {l3}'.
              format(numa=numa_node, path=clear_cache_path, l3=L3_CACHE), check=True, shell=True)

def run_test_host(local_binary_path, vcpus, id, local_test_path=''):
    """Run the benchmark on host."""
    os.environ['OMP_NUM_THREADS'] = str(vcpus)
    os.environ['OMP_PROC_BIND'] = 'close'

    #cset proc --set=firecracker_set --exec 
    proc = run('cset proc --set=firecracker_set --exec ./{binary}'.format(binary=local_binary_path), stdout=PIPE, stderr=PIPE, universal_newlines=True)
    assert proc.stderr == ''
...
def test_memory_performance_1_vcpu(test_microvm_with_ssh, network_config):
    ...
    clear_cache(local_test_path, 1)
    guest_stdout = run_test_guest(ssh_connection, local_binary_path, vcpus)
    clear_cache(local_test_path, 0)
    host_stdout = run_test_host(local_binary_path, vcpus, microvm._microvm_id, local_test_path)
